...
export class ProjetoService {
    ...
    async update(id: number, body: Projeto) {
        if (await this.find(id)) {
            //await this.model.update({ id: id }, body)       // Estava dando erro ao fazer o update, foi trocado pelo create && save que funciona perfeitamente
            try {
                body.id = id; // Para não dar erro de UNIQUE na hora de gravar no banco
                this.validarData(body) // Verifca se a data de inicio é menor que a data de inicio
                await this.validarColaborador(body, id) // Verifica se os colaboradores enviados estão presentes em outros projetos no mesmo período
    ...
    async validarColaborador(body: Projeto, id: number): Promise<Projeto[]> {
